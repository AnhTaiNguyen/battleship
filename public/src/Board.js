...
export default class Board {
  ...
  preload(phaser) {
    ...
    phaser.load.spritesheet("square", "../assets/water3.gif", {
      frameWidth: 48,
      frameHeight: 48,
    });

    // Load aims for explosion when shoot
    phaser.load.spritesheet("hit", "../assets/explosion/hitShip.png", {
      frameWidth: 40,
      frameHeight: 32,
    });

    // Load aims for miss when shoot
    phaser.load.spritesheet("miss", "../assets/explosion/missShip.png", {
      frameWidth: 175,
      frameHeight: 175,
    });

    // Load aims for fire after hit
    phaser.load.spritesheet("fire", "../assets/explosion/Fire2a.gif", {
      frameWidth: 35,
      frameHeight: 40,
    });

    // Setup scence for each square
    this.#board.map((row) => {
      return row.map((cell) => {
